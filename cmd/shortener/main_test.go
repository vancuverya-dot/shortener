...
func TestRoutes(t *testing.T) {
	...
	tests := []struct {
		...
		{"POST root", "POST", "http://localhost:8082/", http.StatusOK},
		{"GET with ID", "GET", "http://localhost:8082/", http.StatusOK},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "POST root" {
				...
				req = httptest.NewRequest(tt.method, tt.path+getId, nil)
			} else {
			...
			if tt.name == "POST root" {
				...
				if w.Body.String() != `https://practicum.yandex.ru/profile/go-advanced/?from=learn_subscriptions-with-prof-recommendations` {
					t.Errorf("got body %s, want %s", w.Body.String(), `https://practicum.yandex.ru/profile/go-advanced/?from=learn_subscriptions-with-prof-recommendations`)
				}
			}
